function display_context_menu(e) {
  ...
  var $this = jQuery(this),
    ...
    is_editable =
      ...
      ($parent.hasClass('ext_afp') ||
        ...
        $parent.hasClass('ext_asp') ||
        $parent.hasClass('ext_aspx') ||
        $parent.hasClass('ext_c') ||
        $parent.hasClass('ext_cfm') ||
        $parent.hasClass('ext_cgi') ||
        $parent.hasClass('ext_cpp') ||
        $parent.hasClass('ext_css') ||
        $parent.hasClass('ext_h') ||
        $parent.hasClass('ext_htm') ||
        $parent.hasClass('ext_html') ||
        $parent.hasClass('ext_js') ||
        $parent.hasClass('ext_lasso') ||
        $parent.hasClass('ext_log') ||
        $parent.hasClass('ext_php') ||
        $parent.hasClass('ext_pl') ||
        $parent.hasClass('ext_py') ||
        $parent.hasClass('ext_rb') ||
        $parent.hasClass('ext_rbx') ||
        $parent.hasClass('ext_rhtml') ||
        $parent.hasClass('ext_ruby') ||
        $parent.hasClass('ext_sql') ||
    ...
    $menu.append(function () {
      var $item = jQuery('<li>')
        ...
        .on('click', function () {
          ...
          simple_ide_set_file_contents($this.attr('rel'), function () {
  ...
  $menu.append(function () {
    var $item = jQuery('<li>')
      ...
      .text('Rename')
      .on('click', function () {
        simple_ide_rename_file($this.attr('rel'), function () {
          alert('Callback');
        });
      });
  ...
  $menu.append(function () {
    var $item = jQuery('<li>')
      ...
      .text('Delete')
      .on('click', function () {
        simple_ide_delete_file($this.attr('rel'), function () {
          alert('Callback');
        });
      });
  ...
  if (is_dir) {
    $menu.append(function () {
      var $item = jQuery('<li>')
        ...
        .text('Upload')
        .on('click', function () {
          simple_ide_upload_file($this.attr('rel'), function () {
            alert('Callback');
          });
        });
    ...
    $menu.append(function () {
      var $item = jQuery('<li>')
        ...
        .text('Download')
        .on('click', function () {
          simple_ide_download_file($this.attr('rel'), function () {
            alert('Callback');
          });
        });
  ...
  if (!is_zip) {
    $menu.append(function () {
      var $item = jQuery('<li>')
        ...
        .text('Zip')
        .on('click', function () {
          simple_ide_zip_file($this.attr('rel'), function () {
            alert('Callback');
          });
        });
    ...
    $menu.append(function () {
      var $item = jQuery('<li>')
        ...
        .text('Unzip')
        .on('click', function () {
          simple_ide_unzip_file($this.attr('rel'), function () {
            alert('Callback');
          });
        });
...
function simple_ide_rename_file(file, callback_func) {
  ...
  jQuery.post(ajaxurl, data, function (response) {
    ...
    if (jQuery("ul.jqueryFileTree a[rel='" + folder + "']").length == 0) {
      ...
      jQuery("ul.jqueryFileTree a[rel='" + folder + "']")
        .click()
        .click();
    }
  });
...
// delete file
function simple_ide_delete_file(file, callback_func) {
  var folder = file.replace(/\/[^\/]*?\/?$/, '/'),
    filename = file.replace(folder, ''),
    data = {
      action: 'simple_ide_delete_file',
      filename: file,
      _wpnonce: jQuery('#_wpnonce').val(),
      _wp_http_referer: jQuery('#_wp_http_referer').val(),
    };

  if (!confirm('Are you sure you wish to delete ' + filename + '?')) return;
...
// upload file
function simple_ide_upload_file(server_destination, callback_func) {
  ...
  jQuery('<input>')
    ...
    .bind('change', function (e) {
      ...
      formdata.append('action', 'simple_ide_upload_file');
        ...
        if (this.readyState === 4) {
          if (this.status === 200) {
            if (this.responseText === '') {
              // If we are adding to wp-content, make sure we refresh the whole tree.
              if (
                jQuery("ul.jqueryFileTree a[rel='" + server_destination + "']")
                  .length == 0
              ) {
                the_filetree();
              } else {
...
// download file
function simple_ide_download_file(file, callback_func) {
  var filename = file.replace(/.*?([^\\\/]+?)$/, '$1'),
    data = {
      action: 'simple_ide_download_file',
      filename: file,
      _wpnonce: jQuery('#_wpnonce').val(),
      _wp_http_referer: jQuery('#_wp_http_referer').val(),
    };
    ...
    $iframe
      ...
      .contentWindow.document.write(
        '<form action="' +
          ...
          ' name="action" value="simple_ide_download_file" /><input type=' +
          '"hidden" name="filename" value="' +
          file +
          '" /><input type="hidden"' +
          ' name="_wpnonce" value="' +
          jQuery('#_wpnonce').val() +
...
// zip given file
function simple_ide_zip_file(file, callback_func) {
  var folder = file.replace(/\/[^\/]*?\/?$/, '/'),
    data = {
      action: 'simple_ide_zip_file',
      filename: file,
      _wpnonce: jQuery('#_wpnonce').val(),
      _wp_http_referer: jQuery('#_wp_http_referer').val(),
    };
...
// unzip given file
function simple_ide_unzip_file(file, callback_func) {
  var folder = file.replace(/\/[^\/]*?\/?$/, '/'),
    data = {
      action: 'simple_ide_unzip_file',
      filename: file,
      _wpnonce: jQuery('#_wpnonce').val(),
      _wp_http_referer: jQuery('#_wp_http_referer').val(),
    };
