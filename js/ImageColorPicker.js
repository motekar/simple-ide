...
(function () {
  var uiImageColorPicker = (function () {
    ...
    var _createImageColorPicker = function (widget) {
      ...
      widget.$canvas = jQuery(
        '<canvas class="ImageColorPickerCanvas"></canvas>'
      );
      widget.$canvas2 = jQuery(
        '<canvas class="ImageColorPickerCanvasColor"></canvas>'
      );
      ...
      if (typeof widget.$canvas.get(0).getContext === 'function') {
        // FF, Chrome, ...
        ...
        if (console) {
          console.log(
            "ImageColor Picker: Can't get canvas context. Use " +
      ...
      try {
        try {
          widget.imageData = widget.ctx.getImageData(
            0,
            0,
            img.width,
            img.height
        ...
        if (console) {
          console.log(
            'ImageColor Picker: Unable to access image data. ' +
    ...
    var imageCoordinates = function (widget, pageX, pageY) {
      ...
      return {
        x: Math.round(pageX - offset.left),
    ...
    var updateCurrentColor = function (widget, red, green, blue, point) {
      ...
      widget.$canvas2.css(
        'top',
        point.y + 30 - jQuery(widget.$canvas).parent().scrollTop()
      );
      widget.$canvas2.css(
        'left',
        point.x + 30 - jQuery(widget.$canvas).parent().scrollLeft()
      );
    };

    var updateSelectedColor = function (widget, red, green, blue) {
      jQuery('#simple_ide_color_assist_input').css(
        'borderRight',
        '30px solid #' + _d2h(red) + _d2h(green) + _d2h(blue)
      );
    ...
    return {
      ...
      selectedColor: function () {
        return (
          '#' + _d2h(this.color[0]) + _d2h(this.color[1]) + _d2h(this.color[2])
        );
      },
    };
  })();
